...
const CATEGORIES = ['All', 'Live Agents', 'Marketing', 'Analytics', 'Documentation', 'HR & Recruitment'];
...
export default function AgentsMarketplace() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.agents?.length > 0) {
            ...
            setAgents([...REAL_AGENTS, ...mapped]);
          } else throw new Error('empty');
        } else throw new Error('api error');
      } catch {
        setAgents([...REAL_AGENTS, ...FALLBACK_AGENTS]);
      } finally {
        setLoading(false);
      }
    };
 ...
 const CATEGORY_COLORS: Record<string, { bg: string; text: string }> = {
    'Live Agents': { bg: 'rgba(16,185,129,0.1)', text: '#059669' },
